...
@Component(label = "ACS AEM Commons - Error Page Handler",
            description = "Error Page Handling module which facilitates the resolution of errors against author-able "
...
public class ErrorPageHandlerImpl implements ErrorPageHandlerService {
    ...
    description = "Error page name (not path) to use if a valid Error Code cannot be retrieved from the Request. "
    ...
    description = "List of valid inclusive content trees under which error pages may reside, along with the name of "
            + "the default error page for the content tree. This is a fallback/less powerful option to adding the "
            + "./errorPages property to CQ Page property dialogs. Example: /content/geometrixx/en:errors [Optional]",
     ...
     * {@inheritDoc}
     *
     * Finds the most appropriate error page to serve using query builder.
     */
    @Override
    public final String findErrorPage(final SlingHttpServletRequest request, final Resource errorResource) {
     ...
     *
     * @param resourceResolver ResourceResolver of the request; used to look up error pages
     * @param pageNames node names to search for
     * @return
     */
    private SearchResult executeQuery(final ResourceResolver resourceResolver, String... pageNames) {
     ...
     */
    private List<String> filterResults(final String rootPath, final SearchResult result) {
     ...
     * {@inheritDoc}
     *
     * Gets the status code via Sling's ERROR_STATUS Request attribute.
     */
    public final int getStatusCode(SlingHttpServletRequest request) {
     ...
     * Currently this implementation only handles status codes and not "Exception name" look-ups.
     */
    public final String getErrorPageName(SlingHttpServletRequest request) {
    ...
    /**
     * Determines if there is a configured System Error Page Path.
     *
     * @return true if there is a valid configured system-wide error page
     */
    private boolean hasSystemErrorPage() {
     ...
     *
     * @return the configured path to the system-wide error page (without extension)
     */
    private String getSystemErrorPagePath() {
     ...
     *
     * @return the configured error page extension
     */
    private String getErrorPageExtension() {
     ...
     *
     * @return a list of configured search paths
     */
    private List<String> getRootPaths(final Map<String, String> errorPagesMap) {
     ...
     *
     * @param rootPath the JCR path to check if it has been configured as a "root path" to look under for error pages
     * @param errorPagesMap the configured map of error pages
     * @return the path to the error page
     */
    private String getErrorPagesPath(final String rootPath, final Map<String, String> errorPagesMap) {
     ...
     *
     * @param resource the requested Resource that caused the error
     * @return the path to the error page
     */
    private String getErrorPagesPath(final Resource resource, final SortedMap<String, String> errorPagesMap) {
    ...
    /**
     * Given the Request path, find the first Real Parent of the Request (even if the resource doesn't exist).
     *
     * This is used to handle the 404 condition (requesting a resource at path that does not exist in the JCR/cannot
     * be resolved to a real resource).
     *
     * @param resource the requested Resource that caused the error
     * @return the closest real resource associated w the request path, or self
     */
     ...
     * Defaults to .html
     *
     * @param path the error page path to apply he extension to
     * @return the path with the error page extension
     */
     ...
     *
     * @param name error page name to search for
     * @return a Query-friendly node name
     */
     ...
     * @return true if the request is anonymous
     */
     ...
     */
    private void authenticateRequest(SlingHttpServletRequest request, SlingHttpServletResponse response) {
     ...
     */
    @Override
    public final void doHandle404(SlingHttpServletRequest request, SlingHttpServletResponse response) {
     ...
     */
    @Override
    public final String getException(SlingHttpServletRequest request) {
     ...
     */
    @Override
    public final String getRequestProgress(SlingHttpServletRequest request) {
        StringWriter stringWriter = new StringWriter();
        if (request != null) {
     ...
     */
    @Override
    public final void resetRequestAndResponse(SlingHttpServletRequest request, SlingHttpServletResponse response,
                                         int statusCode) {
     ...
     *
     * @param master the map with priority; mappings in master supersede those in slave
     * @param slave the map without priority; mappings in master supersede those in slave
     * @return a combined set of the master and slave map, with master entries taking priority
     */
     ...
     * @return
     */
    private SimpleEntry<String, String> toSimpleEntry(final String value, final String separator) {
     ...
     *
     * @param paths error page lookup paths applied via the OSGi configuration
     * @return a map of the content trees (root paths) and their error page trees
     */
    private SortedMap<String, String> configurePathMap(final String[] paths) {
        final SortedMap<String, String> sortedMap = new TreeMap<String, String>(new StringLengthComparator());
        ...
        for (final String path : paths) {
            ...
            final String key = StringUtils.strip((String) tmp.getKey());
