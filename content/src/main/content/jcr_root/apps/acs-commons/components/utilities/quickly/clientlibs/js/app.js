...
angular.element(document).ready(function () {
    ...
    $('html').keypress(function(e){
        var controllerEl = angular.element(document.getElementById(quicklyControllerId));

        if(!controllerEl) {
            return;
        } else if(isQuicklyTogglEvent(e)) {
    ...
    $('html').keydown(function(e){
        ...
        if(isQuicklyDismissEvent(e)) {
            controllerEl = angular.element(document.getElementById(quicklyControllerId));

            if(controllerEl && controllerEl.scope().app.visible) {
                controllerEl.scope().app.toggle();
                controllerEl.scope().$apply();
                e.preventDefault();
            }
    ...
    setTimeout(function() {
        $('iframe').load(function() {
            ...
            iframe.on('keydown keypress', function(e) {
                ...
                if(isQuicklyTogglEvent(e) || isQuicklyDismissEvent(e)) {
                    $('html').trigger(e);
                }
            });
        });
